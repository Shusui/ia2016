from gendata import gen, target
from DBN import *
import sys, numpy
...
def fit(train_data, train_target, pretrain_epochs=100, pretrain_lr=0.1, finetune_epochs=20, finetune_lr=0.1):
	...
	dbn = DBN(input=train_data, label=train_target, hidden_layer_sizes=[1000], n_ins=n_ins, n_outs=n_outs)
...
def perft(dbn, test_data, test_target, to_shuffle, verbose=False):
	...
	for i in range(size):
		...
		if verbose:
			img_name = str(int(to_shuffle[len(to_shuffle)-size+i][0]) + 1) + '.jpg'
...
def classify(perc):
	...
	dbn = fit(train_data, train_target, 10, 0.1, 5, 0.1)
